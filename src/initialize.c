...
int setupSiteEnergies(void) {
	...
	for (int j = 0; j < nrOfpTypes; j++){
		for (int i = 0; i < nrOfSites; i++) {
			siteEnergy[j][i] = random_Normal(DOS_mu[j],DOS_sigma[j]);
		}
	}
	return(0);
}
			...
			if (i != j) {
				...
				if (dist <= rCutOff) {
					...
					for(int pType =0; pType < nrOfpTypes; pType++){
						rateList[pType][i][c_neighbour] = hopRate(dist,  correctedPBC_1D(siteXYZ[j][0]-siteXYZ[i][0], maxXYZ[0]), chargeOfType[pType] , siteEnergy[pType][i], siteEnergy[pType][j]); 
					}
					c_neighbour++;
